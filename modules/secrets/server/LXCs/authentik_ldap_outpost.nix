...
let
  ...
  cfg = config.my.secrets.authentik_ldap_outpost;
in
{

  options.my.secrets.authentik_ldap_outpost = {
  ...
  config = lib.mkIf cfg.enable {
    sops.secrets.authentik_ldap_outpost = {
      sopsFile = ./authentik_ldap_outpost.yaml;
    };
  };
}
