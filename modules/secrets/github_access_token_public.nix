...
let
  ...
  cfg = config.my.secrets.github_access_token_public;
in
{

  options.my.secrets.github_access_token_public = {
    ...
    enable = lib.mkEnableOption "Github token secret for public repo access";
  };

  # lib.mkIf ensures this entire block is ONLY evaluated if cfg.enable is set to true in a host's config.
  config = lib.mkIf cfg.enable {
    sops.secrets.github_access_token_public = {
      sopsFile = ./github_access_token_public.yaml;
    };
  };
}
