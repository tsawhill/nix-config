  ...
  home-manager.users.taylor = {
    ...
    programs.fish = {
      ...
      shellAliases = {
        ...
        rebuild-system = "cd ~/.config/nixos/ && sudo nixos-rebuild switch && git add -A && git commit -m \"$(hostname) - Generation $(nix-env --list-generations | grep current | awk '{print $1}') Successful Build (No flake update)\" && git push ; cd -";
      };
    };
