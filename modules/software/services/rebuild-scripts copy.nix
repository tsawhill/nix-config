...
let
    ...
    text = ''
      ...
      push_host() {
        ...
        if [[ -L "$profile_path" ]]; then
          echo "🚀 Pushing: $host"
          if nixos-rebuild switch --flake "$FLAKE_URI#$host" --target-host "root@$host.lan"; then
             rm "$profile_path"
      ...
      if [[ "$MODE" == "notify" ]]; then
        gotify push -t "❌ CRITICAL: $TASK_NAME Push Failed" -p 10 "$host Deployment failed for 18 consecutive hours."
      fi
    '';
  };
  ...
  mkRebuildTask = name: interval: hosts: {
    ...
    pushService = {
      ...
      onFailure = [ "rebuild-notify-${name}.service" ];
      serviceConfig = {
        Type = "oneshot";
        User = "root";
        Restart = "on-failure";
